...
dbHeader <- dashboardHeader(
	titleWidth = 450,
	title = "Återfynd - Swedish Bird Recoveries",
	disable = FALSE,
...
function(request) {
	body2 <- dashboardBody(tabItems(
		...
		tabItem(tabName = "menu2", uiOutput("birdmap"))
	))
	...
	dashboardPage(
		...
		dashboardSidebar(
			...
			# hr(),
			flowLayout(uiOutput("species"), uiOutput("country")),
			hr(),
			flowLayout(uiOutput("months"), uiOutput("years")),
			hr(),
			flowLayout(uiOutput("lats"), uiOutput("lons")),
			hr(),
			flowLayout(
				uiOutput("source")
